...
function Sidebar({ user }) {
  ...
  const [bukaDataPegawai, setBukaDataPegawai] = useState(
    () => location.pathname === '/data-pegawai'
  )

  const subAktif = (pathname, tabValue, defaultTab = 'ruangan') => {
    if (location.pathname !== pathname) return false
    const tabUrl = new URLSearchParams(location.search).get('tab') || defaultTab
            ...
            {bolehDataPegawai && (
              <>
                <div
                  className="menu-item"
                  style={groupHeaderStyle}
                  onClick={() => setBukaDataPegawai((v) => !v)}
                >
                  <span style={{ flex: 1, display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
                    <IkonUsers size={15} /> Data Pegawai
                  </span>
                  {bukaDataPegawai ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                </div>

                {bukaDataPegawai && (
                  <>
                    <NavLink
                      to="/data-pegawai?tab=masa-kerja"
                      className={kelasSub(subAktif('/data-pegawai', 'masa-kerja', 'masa-kerja'))}
                      style={subIndentStyle}
                    >
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
                        <IkonUsers size={14} /> Masa Kerja
                      </span>
                    </NavLink>
                    <NavLink
                      to="/data-pegawai?tab=kgb"
                      className={kelasSub(subAktif('/data-pegawai', 'kgb', 'masa-kerja'))}
                      style={subIndentStyle}
                    >
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
                        <IkonCalendarClock size={14} /> Kenaikan Gaji Berkala
                      </span>
                    </NavLink>
                  </>
                )}
