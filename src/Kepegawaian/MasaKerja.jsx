...
function MasaKerja({ user }) {
  ...
  const hapusData = (id) => {
    if (
      !window.confirm(
        'Yakin ingin menghapus data Masa Kerja pegawai ini?\n\n' +
          '(Cuma tanggal masuknya yang dikosongkan -- data pegawai, ' +
          'Kenaikan Gaji Berkala, dan fitur lain TIDAK ikut terhapus.)'
      )
    ) {
      return
    }
  ...
  const handleUploadPegawai = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        // Kolom yang didukung: NIP, Nama, Pangkat, Jabatan, Eselon III, Bagian, No HP, Tanggal Masuk
        // Catatan 1: kalau file Excel gak punya kolom "Bagian" tersendiri (kayak file
        ...
        // bukan kode eselon.
        // Catatan 2: kolom "TMT Pangkat" SENGAJA gak dianggap sebagai Tanggal Masuk di sini
        // (itu punya Kenaikan Gaji Berkala) -- header harus jelas "Tanggal Masuk"/"Tgl Masuk"
        // (atau "TMT Masuk"), bukan cuma "TMT" polos, biar gak ketuker sama TMT Pangkat.
        const dataSiapKirim = rows.map((baris) => {
          ...
          return {
            ...
            tanggal_masuk:
              String(
                cari(
                  ['tanggalmasuk', 'tglmasuk', 'tmtmasuk', 'tmt'],
