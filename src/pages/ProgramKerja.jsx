...
function ProgramKerja({ role }) {
  ...
  const programsFiltered = programs.filter((p) => {
    const cocokTahun = filterTahun === 'semua' || p.tahun === Number(filterTahun)
    const cocokBidang = filterBidang === 'semua' || p.bidang === filterBidang
    return cocokTahun && cocokBidang
  })

  const totalProgram = programs.length
  const totalBidang = new Set(programs.map((p) => p.bidang)).size
  const totalSelesai = programs.filter((p) => Object.values(p.realisasi).every((v) => v === 100)).length
  ...
  return (
    <div className="page">
      ...
      <div className="stats-grid">
        <div className="stat-card"><div className="stat-icon">📋</div><div className="stat-info"><h4>Total Program</h4><div className="stat-value">{totalProgram}</div><div className="stat-desc">Program kerja terdaftar</div></div></div>
      ...
      <div className="card">
        ...
        <div className="filter-info">Menampilkan {programsFiltered.length} dari {programs.length} program</div>
              ...
              {programsFiltered.map((p) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td>{role === 'admin' && <button className="btn-danger" onClick={() => hapusProgram(p.id)}>🗑</button>}</td>
                  </tr>
                )
              })}
            </tbody>
